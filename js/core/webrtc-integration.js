...
(function () {
  ...
  // Check if user has premium access — unified Stripe subscription flow
  function isPremiumUser() {
    ...
    // Legacy fallback
    if (localStorage.getItem("user_is_premium") === "true") return true;
    if (sessionStorage.getItem("user_is_premium") === "true") return true;
    return false;
  }
  ...
  window.initWebRTCWidget = function (containerId) {
    ...
    function makeCall(type) {
      ...
      statusDiv.style.cssText =
        "padding: 10px; margin-top: 10px; border-radius: 2rem; text-align: center; font-size: 0.8rem; background: #fef3c7; display: block;";
      ...
      setTimeout(() => {
        statusDiv.style.cssText =
          "padding: 10px; margin-top: 10px; border-radius: 2rem; text-align: center; font-size: 0.8rem; background: #fee2e2; display: block;";
    ...
    function addContact() {
      ...
      if (contactsList) {
        ...
        newContact.style.cssText =
          "background: #f1f5f9; border: none; padding: 6px 14px; border-radius: 20px; cursor: pointer; font-size: 0.8rem;";
